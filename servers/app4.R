...
ui <- fluidPage(
  ...
  tags$head(tags$link(rel = "stylesheet", type = "text/css", href = "https://data.jaxtrib.org/leg/styles.css")),
  div(class = "banner",
...
server <- function(input, output) {
  ...
  output$scatterplot <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = avg_party_lean_points_R, y = leg_party_loyalty, 
      ...
      geom_vline(xintercept = median_info$median_lean, linetype = "dashed", color = "gray50", size = 0.5) +
